...
template < class TDerivedParticle>
TemplatedSphericParticle<TDerivedParticle>::TemplatedSphericParticle()
...
template < class TDerivedParticle>
TemplatedSphericParticle<TDerivedParticle>::TemplatedSphericParticle(IndexType NewId, GeometryType::Pointer pGeometry)
...
template < class TDerivedParticle>
TemplatedSphericParticle<TDerivedParticle>::TemplatedSphericParticle(IndexType NewId, GeometryType::Pointer pGeometry,  PropertiesType::Pointer pProperties)
...
template < class TDerivedParticle>
TemplatedSphericParticle<TDerivedParticle>::TemplatedSphericParticle(IndexType NewId, NodesArrayType const& ThisNodes)
...
template < class TDerivedParticle>
Element::Pointer TemplatedSphericParticle<TDerivedParticle>::Create(IndexType NewId, NodesArrayType const& ThisNodes, PropertiesType::Pointer pProperties) const
{
    return Element::Pointer(new TemplatedSphericParticle(NewId, GetGeometry().Create(ThisNodes), pProperties));
}
...
template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::Initialize(const ProcessInfo& r_process_info)
{
    KRATOS_TRY
...
template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::SetIntegrationScheme(DEMIntegrationScheme::Pointer& integration_scheme) {
    mpIntegrationScheme = integration_scheme->CloneRaw();
}

template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::CalculateRightHandSide(ProcessInfo& r_process_info, double dt, const array_1d<double,3>& gravity, int search_control)
{
    KRATOS_TRY
...
template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::InitializeForceComputation(ProcessInfo& r_process_info){}

template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::FirstCalculateRightHandSide(ProcessInfo& r_process_info, double dt, int search_control){}

template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::CollectCalculateRightHandSide(ProcessInfo& r_process_info){}

template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::FinalCalculateRightHandSide(ProcessInfo& r_process_info, double dt, const array_1d<double,3>& gravity){}

template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::CalculateMaxBallToBallIndentation(double& r_current_max_indentation)
{
    r_current_max_indentation = - std::numeric_limits<double>::max();

    for (unsigned int i = 0; i < mNeighbourElements.size(); i++){
        TemplatedSphericParticle* ineighbour = mNeighbourElements[i];
...
template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::CalculateMaxBallToFaceIndentation(double& r_current_max_indentation)
{
    r_current_max_indentation = - std::numeric_limits<double>::max();
...
template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::CalculateMomentum(array_1d<double, 3>& r_momentum)
{
...
template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::CalculateLocalAngularMomentum(array_1d<double, 3>& r_angular_momentum)
{
...
template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::ComputeNewNeighboursHistoricalData(boost::numeric::ublas::vector<int>& mTempNeighboursIds,
...
template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::ComputeNewRigidFaceNeighboursHistoricalData()
{
    array_1d<double, 3> vector_of_zeros = ZeroVector(3);
    std::vector<DEMWall*>& rNeighbours = this->mNeighbourRigidFaces;
...
template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::EquationIdVector(EquationIdVectorType& rResult, ProcessInfo& r_process_info){}

template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::CalculateMassMatrix(MatrixType& rMassMatrix, ProcessInfo& r_process_info)
{
    rMassMatrix(0,0) = GetMass();
}

template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::EvaluateDeltaDisplacement(double RelDeltDisp[3],
                                                double RelVel[3],
                                                ...
                                                TemplatedSphericParticle* p_neighbour,
                                                double& distance)
{
...
template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::RelativeDisplacementAndVelocityOfContactPointDueToRotation(const double indentation,
...
template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::RelativeDisplacementAndVelocityOfContactPointDueToRotationMatrix(double DeltDisp[3],
                                                double RelVel[3],
                                                double OldLocalCoordSystem[3][3],
                                                const double& other_radius,
                                                const double& dt,
...
template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::ComputeMoments(double NormalLocalContactForce,
                                     double Force[3],
...
template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::ComputeRollingFriction(array_1d<double, 3>& rolling_resistance_moment, double& RollingResistance, double dt)
{
...
template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::ComputeBallToBallContactForce(TemplatedSphericParticle<TDerivedParticle>::ParticleDataBuffer & data_buffer,
...
template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::EvaluateBallToBallForcesForPositiveIndentiations(TemplatedSphericParticle<TDerivedParticle>::ParticleDataBuffer & data_buffer,
                                                                       ...
                                                                       TemplatedSphericParticle* p_neighbour_element,
                                                                       bool& sliding,
                                                                       double LocalCoordSystem[3][3],
                                                                       double OldLocalCoordSystem[3][3],
...
template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::ComputeBallToRigidFaceContactForce(array_1d<double, 3>& r_elastic_force,
                                                         array_1d<double, 3>& r_contact_force,
                                                         double& RollingResistance,
...
template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::RenewData()
{
  //To be redefined
}
template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::ComputeConditionRelativeData(int rigid_neighbour_index,
                                                  DEMWall* const wall,
                                            double LocalCoordSystem[3][3],
                                            double& DistPToB,
...
template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::ComputeWear(double LocalCoordSystem[3][3], array_1d<double, 3>& vel, double tangential_vel[3],
...
template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::CreateDiscontinuumConstitutiveLaws(const ProcessInfo& r_process_info)
{
...
template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::CalculateDampingMatrix(MatrixType& rDampingMatrix, ProcessInfo& r_process_info){}

template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::GetDofList(DofsVectorType& ElementalDofList, ProcessInfo& r_process_info)
{
    KRATOS_TRY
...
template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::InitializeSolutionStep(ProcessInfo& r_process_info)
{
    KRATOS_TRY
...
template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::AddNeighbourContributionToStressTensor(const double Force[3],
                                                             const double other_to_me_vect[3],
                                                             const double distance,
                                                             const double radius_sum,
                                                             TemplatedSphericParticle* element) {
    KRATOS_TRY
...
template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::AddWallContributionToStressTensor(const double Force[3],
                                                        const double other_to_me_vect[3],
                                                        const double distance,
...
template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::CorrectRepresentativeVolume(double& rRepresentative_Volume/*, bool& is_smaller_than_sphere*/) {
...
template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::FinalizeSolutionStep(ProcessInfo& r_process_info){
...
template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::SymmetrizeStressTensor(){
...
template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::ComputeReactions(){
    KRATOS_TRY
    if (this->Is(DEMFlags::HAS_STRESS_TENSOR)) {
...
template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::PrepareForPrinting(ProcessInfo& r_process_info){
...
template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::ComputeAdditionalForces(array_1d<double, 3>& externally_applied_force,
...
template < class TDerivedParticle>
array_1d<double,3> TemplatedSphericParticle<TDerivedParticle>::ComputeWeight(const array_1d<double,3>& gravity, const ProcessInfo& r_process_info) {
...
template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::AddUpForcesAndProject(double OldCoordSystem[3][3],
                                            double LocalCoordSystem[3][3],
...
template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::AddUpMomentsAndProject(double LocalCoordSystem[3][3],
...
template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::AddUpFEMForcesAndProject(double LocalCoordSystem[3][3],
                                               double LocalContactForce[3],
                                               double LocalElasticContactForce[3],
                                               double GlobalContactForce[3],
                                               double GlobalElasticContactForce[3],
...
template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::MemberDeclarationFirstStep(const ProcessInfo& r_process_info)
...
template < class TDerivedParticle>
double TemplatedSphericParticle<TDerivedParticle>::CalculateLocalMaxPeriod(const bool has_mpi, const ProcessInfo& r_process_info) {
    KRATOS_TRY
...
template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::ComputeOtherBallToBallForces(array_1d<double, 3>& other_ball_to_ball_forces) {}


template < class TDerivedParticle>
double TemplatedSphericParticle<TDerivedParticle>::GetInitialDeltaWithFEM(int index) //only available in continuum_particle
{
    return 0.0;
}

template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::Calculate(const Variable<double>& rVariable, double& Output, const ProcessInfo& r_process_info)
{
    KRATOS_TRY
...
template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::Calculate(const Variable<array_1d<double, 3> >& rVariable, array_1d<double, 3>& Output,
                                const ProcessInfo& r_process_info)
{
...
template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::RotateOldContactForces(const double OldLocalCoordSystem[3][3], const double LocalCoordSystem[3][3], array_1d<double, 3>& mNeighbourElasticContactForces) {
...
template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::CalculateRelativePositions(ParticleDataBuffer & data_buffer)
{
...
template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::RelativeDisplacementAndVelocityOfContactPointDueToOtherReasons(const ProcessInfo& r_process_info,
                                                                                    double DeltDisp[3], //IN GLOBAL AXES
                                                                                    double RelVel[3], //IN GLOBAL AXES
                                                                                    double OldLocalCoordSystem[3][3],
                                                                                    double LocalCoordSystem[3][3],
                                                                                    TemplatedSphericParticle* neighbour_iterator) {}

template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::SendForcesToFEM(){}


template < class TDerivedParticle>
bool TemplatedSphericParticle<TDerivedParticle>::GetDomainPeriodicity()
{
...
template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::TransformToClosestPeriodicCoordinates(double my_coors[3], double other_coors[3])
{
   DiscreteParticleConfigure<3>::TransformToClosestPeriodicCoordinates(my_coors, other_coors);
}

template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::Move(const double delta_t, const bool rotation_option, const double force_reduction_factor, const int StepFlag) {
    GetIntegrationScheme().Move(GetGeometry()[0], delta_t, rotation_option, force_reduction_factor, StepFlag);
}

template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::Calculate(const Variable<Vector >& rVariable, Vector& Output, const ProcessInfo& r_process_info){}
template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::Calculate(const Variable<Matrix >& rVariable, Matrix& Output, const ProcessInfo& r_process_info){}

template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::AdditionalCalculate(const Variable<double>& rVariable, double& Output, const ProcessInfo& r_process_info){}

template < class TDerivedParticle>
void TemplatedSphericParticle<TDerivedParticle>::ApplyGlobalDampingToContactForces() {
...
template < class TDerivedParticle>
int    TemplatedSphericParticle<TDerivedParticle>::GetClusterId()                                                           { return mClusterId;      }
template < class TDerivedParticle>
void   TemplatedSphericParticle<TDerivedParticle>::SetClusterId(int givenId)                                                { mClusterId = givenId;   }
template < class TDerivedParticle>
double TemplatedSphericParticle<TDerivedParticle>::GetRadius()                                                              { return mRadius;         }
template < class TDerivedParticle>
double TemplatedSphericParticle<TDerivedParticle>::CalculateVolume()                                                        { return 4.0 * KRATOS_M_PI_3 * mRadius * mRadius * mRadius;     }
template < class TDerivedParticle>
void   TemplatedSphericParticle<TDerivedParticle>::SetRadius(double radius)                                                 { mRadius = radius;       }
template < class TDerivedParticle>
void   TemplatedSphericParticle<TDerivedParticle>::SetRadius()                                                              { mRadius = GetGeometry()[0].FastGetSolutionStepValue(RADIUS);       }
template < class TDerivedParticle>
double TemplatedSphericParticle<TDerivedParticle>::GetInteractionRadius(const int radius_index)                             { return mRadius;         }
template < class TDerivedParticle>
void   TemplatedSphericParticle<TDerivedParticle>::SetInteractionRadius(const double radius, const int radius_index)        { mRadius = radius; GetGeometry()[0].FastGetSolutionStepValue(RADIUS) = radius;}
template < class TDerivedParticle>
double TemplatedSphericParticle<TDerivedParticle>::GetSearchRadius()                                                        { return mSearchRadius;   }
template < class TDerivedParticle>
double TemplatedSphericParticle<TDerivedParticle>::GetSearchRadiusWithFem()                                                 { return mSearchRadiusWithFem;   }
template < class TDerivedParticle>
void   TemplatedSphericParticle<TDerivedParticle>::SetSearchRadius(const double radius)                                     { mSearchRadius = radius; }
template < class TDerivedParticle>
void   TemplatedSphericParticle<TDerivedParticle>::SetSearchRadiusWithFem(const double radius)                              { mSearchRadiusWithFem = radius; }
template < class TDerivedParticle>
double TemplatedSphericParticle<TDerivedParticle>::GetMass()                                                                { return mRealMass;       }
template < class TDerivedParticle>
void   TemplatedSphericParticle<TDerivedParticle>::SetMass(double real_mass)                                                { mRealMass = real_mass;  GetGeometry()[0].FastGetSolutionStepValue(NODAL_MASS) = real_mass;}
template < class TDerivedParticle>
double TemplatedSphericParticle<TDerivedParticle>::CalculateMomentOfInertia()                                               { return 0.4 * GetMass() * GetRadius() * GetRadius(); }

template < class TDerivedParticle>
double TemplatedSphericParticle<TDerivedParticle>::GetYoung()                                                               { return GetFastProperties()->GetYoung();                     }
template < class TDerivedParticle>
double TemplatedSphericParticle<TDerivedParticle>::GetRollingFriction()                                                     { return GetFastProperties()->GetRollingFriction();           }
template < class TDerivedParticle>
double TemplatedSphericParticle<TDerivedParticle>::GetPoisson()                                                             { return GetFastProperties()->GetPoisson();                   }
template < class TDerivedParticle>
double TemplatedSphericParticle<TDerivedParticle>::GetTgOfFrictionAngle()                                                   { return GetFastProperties()->GetTgOfFrictionAngle() ;        }
template < class TDerivedParticle>
double TemplatedSphericParticle<TDerivedParticle>::GetCoefficientOfRestitution()                                            { return GetFastProperties()->GetCoefficientOfRestitution();  }
template < class TDerivedParticle>
double TemplatedSphericParticle<TDerivedParticle>::GetLnOfRestitCoeff()                                                     { return GetFastProperties()->GetLnOfRestitCoeff();           }
template < class TDerivedParticle>
double TemplatedSphericParticle<TDerivedParticle>::GetDensity()                                                             { return GetFastProperties()->GetDensity();                   }
template < class TDerivedParticle>
int    TemplatedSphericParticle<TDerivedParticle>::GetParticleMaterial()                                                    { return GetFastProperties()->GetParticleMaterial();          }
template < class TDerivedParticle>
double TemplatedSphericParticle<TDerivedParticle>::GetParticleCohesion()                                                    { return GetFastProperties()->GetParticleCohesion();          }
template < class TDerivedParticle>
double TemplatedSphericParticle<TDerivedParticle>::GetParticleKNormal()                                                     { return GetFastProperties()->GetParticleKNormal();           }
template < class TDerivedParticle>
double TemplatedSphericParticle<TDerivedParticle>::GetParticleKTangential()                                                 { return GetFastProperties()->GetParticleKTangential();       }

// Conical damage
template < class TDerivedParticle>
double TemplatedSphericParticle<TDerivedParticle>::GetParticleContactRadius()                                               { return GetFastProperties()->GetParticleContactRadius();     }
template < class TDerivedParticle>
double TemplatedSphericParticle<TDerivedParticle>::GetParticleMaxStress()                                                   { return GetFastProperties()->GetParticleMaxStress();         }
template < class TDerivedParticle>
double TemplatedSphericParticle<TDerivedParticle>::GetParticleAlpha()                                                       { return GetFastProperties()->GetParticleAlpha();             }
template < class TDerivedParticle>
double TemplatedSphericParticle<TDerivedParticle>::GetParticleGamma()                                                       { return GetFastProperties()->GetParticleGamma();             }

template < class TDerivedParticle>
array_1d<double, 3>& TemplatedSphericParticle<TDerivedParticle>::GetForce()                                                 { return GetGeometry()[0].FastGetSolutionStepValue(TOTAL_FORCES);}
template < class TDerivedParticle>
double&              TemplatedSphericParticle<TDerivedParticle>::GetElasticEnergy()                                         { return mElasticEnergy; }
template < class TDerivedParticle>
double&              TemplatedSphericParticle<TDerivedParticle>::GetInelasticFrictionalEnergy()                             { return mInelasticFrictionalEnergy; }
template < class TDerivedParticle>
double&              TemplatedSphericParticle<TDerivedParticle>::GetInelasticViscodampingEnergy()                           { return mInelasticViscodampingEnergy; }

template < class TDerivedParticle>
void   TemplatedSphericParticle<TDerivedParticle>::SetYoungFromProperties(double* young)                                    { GetFastProperties()->SetYoungFromProperties( young);                             }
template < class TDerivedParticle>
void   TemplatedSphericParticle<TDerivedParticle>::SetRollingFrictionFromProperties(double* rolling_friction)               { GetFastProperties()->SetRollingFrictionFromProperties( rolling_friction);        }
template < class TDerivedParticle>
void   TemplatedSphericParticle<TDerivedParticle>::SetPoissonFromProperties(double* poisson)                                { GetFastProperties()->SetPoissonFromProperties( poisson);                         }
template < class TDerivedParticle>
void   TemplatedSphericParticle<TDerivedParticle>::SetTgOfFrictionAngleFromProperties(double* tg_of_friction_angle)         { GetFastProperties()->SetTgOfFrictionAngleFromProperties( tg_of_friction_angle);  }
template < class TDerivedParticle>
void   TemplatedSphericParticle<TDerivedParticle>::SetCoefficientOfRestitutionFromProperties(double* coefficient_of_restitution)     { GetFastProperties()->SetCoefficientOfRestitutionFromProperties( coefficient_of_restitution);      }
template < class TDerivedParticle>
void   TemplatedSphericParticle<TDerivedParticle>::SetLnOfRestitCoeffFromProperties(double* ln_of_restit_coeff)     { GetFastProperties()->SetLnOfRestitCoeffFromProperties( ln_of_restit_coeff);      }
template < class TDerivedParticle>
void   TemplatedSphericParticle<TDerivedParticle>::SetDensityFromProperties(double* density)                        { GetFastProperties()->SetDensityFromProperties( density);                         }
template < class TDerivedParticle>
void   TemplatedSphericParticle<TDerivedParticle>::SetParticleMaterialFromProperties(int* particle_material)        { GetFastProperties()->SetParticleMaterialFromProperties( particle_material);      }
template < class TDerivedParticle>
void   TemplatedSphericParticle<TDerivedParticle>::SetParticleCohesionFromProperties(double* particle_cohesion)     { GetFastProperties()->SetParticleCohesionFromProperties( particle_cohesion);      }
template < class TDerivedParticle>
void   TemplatedSphericParticle<TDerivedParticle>::SetParticleKNormalFromProperties(double* particle_k_normal)      { GetFastProperties()->SetParticleKNormalFromProperties( particle_k_normal);       }
template < class TDerivedParticle>
void   TemplatedSphericParticle<TDerivedParticle>::SetParticleKTangentialFromProperties(double* particle_k_tangential) { GetFastProperties()->SetParticleKTangentialFromProperties( particle_k_tangential); }

// Conical damage
template < class TDerivedParticle>
void   TemplatedSphericParticle<TDerivedParticle>::SetParticleContactRadiusFromProperties(double* particle_contact_radius) { GetFastProperties()->SetParticleContactRadiusFromProperties( particle_contact_radius); }
template < class TDerivedParticle>
void   TemplatedSphericParticle<TDerivedParticle>::SetParticleMaxStressFromProperties(double* particle_max_stress)  { GetFastProperties()->SetParticleMaxStressFromProperties( particle_max_stress);   }
template < class TDerivedParticle>
void   TemplatedSphericParticle<TDerivedParticle>::SetParticleAlphaFromProperties(double* particle_alpha)           { GetFastProperties()->SetParticleAlphaFromProperties( particle_alpha);            }
template < class TDerivedParticle>
void   TemplatedSphericParticle<TDerivedParticle>::SetParticleGammaFromProperties(double* particle_gamma)           { GetFastProperties()->SetParticleGammaFromProperties( particle_gamma);            }

template < class TDerivedParticle>
PropertiesProxy* TemplatedSphericParticle<TDerivedParticle>::GetFastProperties()                                    { return mFastProperties;                                                          }
template < class TDerivedParticle>
void   TemplatedSphericParticle<TDerivedParticle>::SetFastProperties(PropertiesProxy* pProps)                       { mFastProperties = pProps;                                                        }
template < class TDerivedParticle>
void   TemplatedSphericParticle<TDerivedParticle>::SetFastProperties(std::vector<PropertiesProxy>& list_of_proxies) {
...
template < class TDerivedParticle>
double TemplatedSphericParticle<TDerivedParticle>::SlowGetYoung()                                                   { return GetProperties()[YOUNG_MODULUS];                                           }
template < class TDerivedParticle>
double TemplatedSphericParticle<TDerivedParticle>::SlowGetRollingFriction()                                         { return GetProperties()[ROLLING_FRICTION_OPTION];                                 }
template < class TDerivedParticle>
double TemplatedSphericParticle<TDerivedParticle>::SlowGetPoisson()                                                 { return GetProperties()[POISSON_RATIO];                                           }
template < class TDerivedParticle>
double TemplatedSphericParticle<TDerivedParticle>::SlowGetTgOfFrictionAngle()                                       { return GetProperties()[PARTICLE_FRICTION];                                       }
template < class TDerivedParticle>
double TemplatedSphericParticle<TDerivedParticle>::SlowGetCoefficientOfRestitution()                                { return GetProperties()[COEFFICIENT_OF_RESTITUTION];                              }
template < class TDerivedParticle>
double TemplatedSphericParticle<TDerivedParticle>::SlowGetDensity()                                                 { return GetProperties()[PARTICLE_DENSITY];                                        }
template < class TDerivedParticle>
int    TemplatedSphericParticle<TDerivedParticle>::SlowGetParticleMaterial()                                        { return GetProperties()[PARTICLE_MATERIAL];                                       }
template < class TDerivedParticle>
double TemplatedSphericParticle<TDerivedParticle>::SlowGetParticleCohesion()                                        { return GetProperties()[PARTICLE_COHESION];                                       }
template < class TDerivedParticle>
double TemplatedSphericParticle<TDerivedParticle>::GetBoundDeltaDispSq()                                            { return mBoundDeltaDispSq;   }
