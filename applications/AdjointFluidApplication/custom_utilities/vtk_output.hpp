...
*/
  ...
  private:
    ...
    struct stat info;
        ...
        if ( stat( "./vtk_output", &info) == 0)
        {
            if (!(info.st_mode & S_IFDIR) )
            {
                KRATOS_THROW_ERROR(std::runtime_error, "vtk_output folder not found. Please create \"vtk_output\" folder.", "")
            }
        }
        else
        {
            KRATOS_THROW_ERROR(std::runtime_error, "vtk_output folder not accessible. Please make \"vtk_output\" folder accessible", "")
        }

        std::string outputFilename = "./vtk_output/" + model_part.Name() + "_" + std::to_string(rank) + "_" + std::to_string(step) + ".vtk";
