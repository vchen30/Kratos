...
// 	}
	...
	void ShellThickElement3D3N::CalculateMassMatrix(MatrixType& rMassMatrix, ProcessInfo& rCurrentProcessInfo)
	{
		...
		else // Lumped mass matrix
		{
			...
			for (SizeType i = 0; i < 3; i++) // loop on nodes
			{
				SizeType index = i * 6;
		...
		const ProcessInfo& rCurrentProcessInfo)
	{
		...
		if (caseId > 19) // calculate stresses
		{
			...
			for (SizeType gauss_point = 0; gauss_point < OPT_NUM_GP; ++gauss_point)
			{
	...
	void ShellThickElement3D3N::SetCrossSectionsOnIntegrationPoints(std::vector< ShellCrossSection::Pointer >& rCrossSections)
	{
		KRATOS_TRY
	...
	double ShellThickElement3D3N::CalculateTsaiWuPlaneStress(const CalculationData & data, const Matrix& rLamina_Strengths, const SizeType& rPly)
	{
	...
	void ShellThickElement3D3N::AddBodyForces(CalculationData& data, VectorType& rRightHandSideVector)
	{
		...
		*/
		...
		//for (SizeType igauss = 0; igauss < OPT_NUM_GP; igauss++)
		for (SizeType igauss = 0; igauss < 1; igauss++)
		{
