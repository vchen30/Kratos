...
namespace Kratos
{
        ...
        ProcessInfo& rCurrentProcessInfo )
    {
        ...
        unsigned int vectorSize = dim*NumberOfNodes;
        ...
        {
            rResult.resize(vectorSize,false);
        }
        ...
        if(dim == 2)
        {
            for (unsigned int i = 0; i < NumberOfNodes; ++i)
            {
                ...
                if (this->HasRotDof()) index = i*3;
                rResult[index    ] = GetGeometry()[i].GetDof(DISPLACEMENT_X,pos    ).EquationId();
                rResult[index + 1] = GetGeometry()[i].GetDof(DISPLACEMENT_Y,pos + 1).EquationId();
                if (this->HasRotDof()) rResult[index + 2] = GetGeometry()[i].GetDof(ROTATION_Z,pos + 2).EquationId();
        ...
        else
        {
            for (unsigned int i = 0; i < NumberOfNodes; ++i)
            {
                ...
                if (this->HasRotDof()) index = i*6;
                rResult[index    ] = GetGeometry()[i].GetDof(DISPLACEMENT_X,pos    ).EquationId();
                rResult[index + 1] = GetGeometry()[i].GetDof(DISPLACEMENT_Y,pos + 1).EquationId();
                rResult[index + 2] = GetGeometry()[i].GetDof(DISPLACEMENT_Z,pos + 2).EquationId();
                if (this->HasRotDof())
                {
                    rResult[index + 3] = GetGeometry()[i].GetDof(ROTATION_X,pos    ).EquationId();
                    rResult[index + 4] = GetGeometry()[i].GetDof(ROTATION_Y,pos + 1).EquationId();
                    rResult[index + 5] = GetGeometry()[i].GetDof(ROTATION_Z,pos + 2).EquationId();                    
                }
            }
        }
        KRATOS_CATCH("")
        ...
        )
    {
        ...
        if(dim == 2)
        {
            for (unsigned int i = 0; i < NumberOfNodes; ++i)
            {
                ...
                if (this->HasRotDof()) ElementalDofList.push_back( GetGeometry()[i].pGetDof(ROTATION_Z));
            }
        }
        else
        {
